...
sfont = pygame.font.SysFont('Arial', 10, bold=True)
...
def main():
    ...
    def main_logic():
        ...
        #starting weapon
        if player.name == "Tuta":
            force_field = Force_Field(player.rect.centerx,player.rect.centery,player)
        ...
        while running:
            ...
            screen.blit(lv_text,(exp_bar_rect.x-100,exp_bar_rect.y-7))

            #kill count text
            kill_text = font.render(f"Kill: {player.kill_count}", True, WHITE)
    ...
    def game_time(time_lag):
        current_time = ((pygame.time.get_ticks()-time_lag) // 1000)
...
class RedMushroom(Equipment):
    ...
    def update(self,player):
        self.plus = player.AREA*1.1*self.LV
        if self in player.equipments:
...
class LightCarrot(Equipment):
    ...
    def update(self,player):
        self.plus = player.MVMSPD*1.05*self.LV
        if self in player.equipments:
